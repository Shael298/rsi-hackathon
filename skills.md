# Curator Operating Manual — Aptura RSI Hack
...
→ **Keep every version.** `submissions/shael-alex/<domain>/archive/vN.SKILL.md`, plus a
...
```sh
uv run stbench check-skill submissions/shael-alex/health
uv run stbench eval --domain health --skill submissions/shael-alex/health \
...
```sh
uv run stbench check-skill submissions/shael-alex/<domain>
uv run stbench eval --domain <domain> --skill submissions/shael-alex/<domain> \
...
5. Diff against the partner's Fable-curated `submissions/shael-alex-b/` and submit the
   higher net_delta per domain.
